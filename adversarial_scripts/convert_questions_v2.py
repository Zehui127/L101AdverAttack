...
#TODO CHANGE
  def convert(self, q, a, tokens, const_parse, run_fix_style=True):
    ...
    if a == "":
      a = RULE_FOR_NOANSWER[self.target]
...
def print_answers(qas):
  for question, answers, context in tqdm(qas):
    toks = list(answers)
    toks = [ele['text'] for ele in toks]
...
def run_corenlp(dataset, qas):
  cache = {}
  print 'Start the conversion process... in run_corenlp'
...
def get_answer_sentence_start_end(context,ans_start):
  """return the start and end position of the sentence where the answer locates"""
  #use the sliding window approach
  pos_list = []
  pos_list = [(0,0)]
  for i in range(len(context)):
    if context[i] == "." or context[i]==",":
      pos_list[-1] = (pos_list[-1][0],i)
      pos_list.append((i,i))
  res = pos_list[:-1][1:]
  return [(x,y) for x,y in res if x >ans_start or y<ans_start]
def closest_insertion(context,ans_start):
  """return the position where we can insert sentence to"""
  #use the sliding window approach
  pos_list = []
  pos_list = [(0,0)]
  con_len = len(context)
  for i in range(con_len):
    if context[i] == "." or context[i]==",":
      ...
      pos_list.append((i,i))
  res = pos_list[:-2]
  # try to find an index where we can insert the generated sentence into the context paragraph
  for x,y in res:
    if x < ans_start and y> ans_start:
      return y
  # if ans == [] or ans in last sentence omit
  return
def insert_sentence(context,sentence,index):
  pre_context = context[:index+1]
  post_context = context[index:]
  sentence = '%s %s' % ('by the way',sentence)
  new_context = '%s %s%s' % (pre_context,sentence,post_context)
  return new_context
import random
...
def punct_attack(cur_text,sentence_boundary,num_perturb=6):
  """Randomly select four positions from the context to to do punctuation attacking"""
  size_len = len(sentence_boundary)
  for i in range(num_perturb):
    index = random.randrange(size_len)
    index_key = sentence_boundary[index][1]
    punct_index = random.randrange(4)
    pre_context = cur_text[:index_key]
    post_context = cur_text[index_key+1:]
    cur_text = '%s%s%s' % (pre_context,PUNCT_SET[punct_index],post_context)
  return cur_text
def ordered_attack(cur_text,sentence_boundary,nearby_word_dict,postag_dict,order=1):
    for x,y in sentence_boundary:
      pre_sent = cur_text[:x+1]
      post_sent = cur_text[y:]
      sent = cur_text[x+1:y]
      tokens = sent.split(" ")
      new_sentence = ""
      for ele in range(order):
        for tok_inx in range(len(tokens)):
          tok = tokens[tok_inx]
          replace_word = tok
          if tok in nearby_word_dict:
            if tok not in STOPWORDS_SET:
              for i in range(1,5):
                temp = nearby_word_dict[tok][i]['word']
                if temp in postag_dict and tok in postag_dict:
                  if postag_dict[temp] == postag_dict[tok] and postag_dict[tok]!='CD':
                    replace_word = temp
                    print(temp,tok)
                    #print(tok,temp,postag_dict[temp])
                    break
          if len(replace_word) <=2:
            new_sentence = " ".join(tokens)
          elif tok_inx == len(tokens)-1:
            new_sentence = '%s %s' % (" ".join(tokens[:tok_inx]),replace_word)
          elif tok_inx == 0:
            new_sentence = '%s %s' % (replace_word," ".join(tokens[tok_inx+1:]))
          else:
            new_sentence ='%s %s %s' % (" ".join(tokens[:tok_inx]),replace_word," ".join(tokens[tok_inx+1:]))
          tokens = new_sentence.split(" ")
      cur_text = '%s%s%s' % (pre_sent,new_sentence,post_sent)
    return cur_text
def replace_version3(cur_text,sentence_boundary,type_args,nearby_word_dict,postag_dict):
  if type_args == "punct":
    return punct_attack(cur_text,sentence_boundary)
  if type_args == "word_flip":
    return ordered_attack(cur_text,sentence_boundary,nearby_word_dict,postag_dict,1)
def dump_data2(dataset, prefix, use_answer_placeholder=False, alteration_strategy=None):
  corenlp_cache = load_cache()
  nearby_word_dict = load_nearby_words()
  postag_dict = load_postag_dict()
  out_data = []
        ...
        for q_str, q_tokens, q_const_parse, tag in q_list:
          for rule in CONVERSION_RULES:
            ...
            if sent:
              ...
              else:#TODO Make Change for version three
                # punctuation attacking and word fliping
                cur_text = paragraph['context']
                ans_start = -1
                if len(qa['answers']) > 0:
                  ans_start = qa['answers'][0]['answer_start']
                sentence_boundary = get_answer_sentence_start_end(cur_text,ans_start)
                if not len(sentence_boundary)==0:
                  cur_text = replace_version3(cur_text,sentence_boundary,"word_flip",nearby_word_dict,postag_dict)
                # Doc2Vec to increase the efficiency of adversarial examples
          ...
          for rule in CONVERSION_RULES:
            ...
            if sent:
              ...
              if OPTS.prepend:
                ...
                cur_qa['answers'] = new_answers
              else:#TODO Make Change for version two
                """cur_text = '%s %s %s' % (sent, paragraph['context'], sent)
                new_answers = []
                for a in qa['answers']:
                  new_answers.append({
                      'text': a['text'],
                ...
                cur_qa['answers'] = new_answers"""
                #TODO Make Change for version two 
                cur_text = '%s %s %s' % (sent, paragraph['context'], sent)
                new_answers = []
                ans_max = 0#get the max index of answers
                for a in qa['answers']:
                  temp = a['answer_start'] + len(sent) + 1
                  new_answers.append({
                      'text': a['text'],
                      'answer_start': temp
                  })
                  if temp>ans_max:
                    ans_max = temp
                #insert into the context, cloest postpend
                ind_insert = closest_insertion(cur_text,ans_max)
                if ind_insert:
                  cur_text = insert_sentence(cur_text,sent[:-1],ind_insert)
                cur_qa['answers'] = new_answers
...
def main():
  print 'Start reading in file..'
  dataset = read_data()
  qas = get_qas(dataset)
  print 'Finish reading in the data...'
  if OPTS.modified_answers:
    global ANSWER_RULES
  ...
  if OPTS.command == 'print-questions':
   ...
   generate_answers(qas)
    ...
    dump_data2(dataset, 'convHighConf', alteration_strategy='high-conf')
...
if __name__ == '__main__':
  print >> sys.stderr, 'Start the program...'
  OPTS = parse_args()
  print >> sys.stderr, 'Finish argument parsing...'
  main()
